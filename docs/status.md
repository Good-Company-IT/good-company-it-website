...
- ✅ **French removed and toptiertrader leftovers deleted** (2026-09-18) — `/fr/about|services|community|contact` were returning HTTP 500 in production (undeclared `metadata_fr`); French is not planned, so the whole locale was removed and `/fr/*` now 308-redirects to `/en/*`. Also deleted the dead promo banner/popup for another company (`toptiertrader`) and its unused translation keys. The sitemap that still lists toptiertrader.com URLs is deferred to the redesign — see `pending.md`.
